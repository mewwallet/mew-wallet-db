...
extension OptionSet where Self: OptionSetAssociated, Self == Element {
  ...
  public mutating func insert(_ newMember: Element) -> (inserted: Bool, memberAfterInsert: Element) {
    ...
    if shouldInsert {
      ...
      let value = newMember.store[newMember.rawValue]
  ...
  public mutating func remove(_ member: Element) -> Element? {
    ...
    let value = store[member.rawValue]
  ...
  public mutating func update(with newMember: Element) -> Element? {
    let previousValue = self.store[newMember.rawValue]
...
extension OptionSetAssociated {
  public subscript(for member: Self) -> Self.AT? {
    self.store[member.rawValue]
  }
}
