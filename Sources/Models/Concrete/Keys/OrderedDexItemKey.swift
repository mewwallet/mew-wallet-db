...
public class OrderedDexItemKey: MDBXKey {
  ...
  private lazy var _order: UInt16 = {
    let value = key[_orderRange].withUnsafeBytes {
  ...
  public init(chain: MDBXChain, order: UInt16, contractAddress: String) {
    ...
    let order               = withUnsafeBytes(of: order.bigEndian) { Data($0) }.setLengthLeft(MDBXKeyLength.order)
    let contractAddressPart = Data(hex: contractAddress).setLengthLeft(MDBXKeyLength.contractAddress)
    
    self.key = chainPart + order + contractAddressPart
  }
