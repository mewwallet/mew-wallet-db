...
final class market_collection_tests: XCTestCase {
  ...
  func testMarketCollection() async {
    do {
      ...
      XCTAssertEqual(marketCollectionItem1.tokens[0].price, Decimal(wrapped: item1.tokens[0]._wrapped.price, hex: false))
  ...
  func testMarketMovers() async {
    do {
      ...
      // read key with 1
      let keyIndex1 = MarketMoversItemKey(chain: chain, timestamp: response.items[1].timestamp, index: 1)
      XCTAssertEqual(keyIndex1.index, 1)
      XCTAssertEqual(UInt64(keyIndex1.timestamp.timeIntervalSince1970), UInt64(response.items[1].timestamp.timeIntervalSince1970))
      ...
      XCTAssertEqual(item1.price, marketMoversItem1.price)

      
      // non-existing key
      let nonexistingCurrency = MarketMoversItemKey(chain: chain, timestamp: Date(), index: 1)
      do {
      ...
      let nonexistingSort = MarketMoversItemKey(chain: chain, timestamp: Date(), index: 1)
      do {
      ...
      let nonexistingChain = MarketMoversItemKey(chain: .polygon_mumbai, timestamp: item1.timestamp, index: 1)
      do {
  ...
  func testMarketList() async {
    do {
      ...
      // read key with 1
      let keyIndex1 = MarketTopItemKey(chain: chain, currency: "USD", sort: "marketCap", index: 1)
      ...
      XCTAssertEqual(item1._wrapped, marketItem1._wrapped)

      // lowest range
      let startKey = MarketTopItemKey(chain: chain, currency: "USD", sort: "marketCap", lowerRange: true)
      let endKey = MarketTopItemKey(chain: chain, currency: "USD", sort: "marketCap", lowerRange: false)
      ...
      for (index, value) in moversItem.enumerated() {
        XCTAssertEqual(response.items[index]._wrapped, value._wrapped)
      }
    } catch {
      XCTFail(error.localizedDescription)
    }
