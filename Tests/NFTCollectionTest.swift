...
final class nft_collection_tests: XCTestCase {
  ...
  func test() async {
    do {
      ...
      keysAndObjects = try objects.collectMetas(chain: .universal).lazy.map { ($0.key, $0) }
      ...
      XCTAssertFalse(asset.isFavorite(chain: .universal))
      XCTAssertFalse(asset.isHidden(chain: .universal))
      ...
      XCTAssertTrue(asset.isFavorite(chain: .universal))
      XCTAssertFalse(asset.isHidden(chain: .universal))
      ...
      XCTAssertTrue(asset.isFavorite(chain: .universal))
      XCTAssertTrue(asset.isHidden(chain: .universal))
      ...
      XCTAssertFalse(asset.isFavorite(chain: .universal))
      XCTAssertTrue(asset2.isFavorite(chain: .universal))
      ...
      XCTAssertEqual([asset2.key.key.hexString], try freshAccount.nftFavorite(chain: .universal).map({ $0.key.key.hexString }))
      ...
      XCTAssertEqual([asset2.key.key.hexString, asset.key.key.hexString], try freshAccount.nftFavorite(chain: .universal).map({ $0.key.key.hexString }))
      ...
      XCTAssertTrue(asset2.isFavorite(chain: .universal))
      freshAccount = try db.read(key: AccountKey(address: .unknown("0x4Dd2a335d53BCD17445EBF4504c5632c13A818A1")), table: .account)
      XCTAssertEqual([asset.key.key.hexString, asset2.key.key.hexString], try freshAccount.nftFavorite(chain: .universal).map({ $0.key.key.hexString }))
      
      debugPrint(try freshAccount.nftFavorite(chain: .universal))
